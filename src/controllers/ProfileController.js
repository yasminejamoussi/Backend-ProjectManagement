...
const pdfjsLib = require("pdfjs-dist"); // Add for better PDF parsing if needed
  ...
  cvUpload(req, res, async (err) => {
    ...
    try {
      ...
      try {
        ...
        const pdfData = await pdfParse(pdfBuffer, { max: 10, ignoreErrors: true });
        cvText = pdfData.text || "";
        console.log("✅ Texte extrait complet par pdf-parse :", cvText.substring(0, 200) + "...");
        ...
        if (!cvText || cvText.trim().length === 0) {
          ...
          try {
            ...
            console.log("✅ Texte extrait par pdftotext :", cvText.substring(0, 200) + "...");
